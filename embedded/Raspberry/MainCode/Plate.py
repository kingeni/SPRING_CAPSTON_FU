...
def loadLabel():
    ...
    for class_ in os.listdir('Train'):
        ...
        label[int(this_class)]=this_label
    return label
labelSet = loadLabel()
...
def predict(detectedChar):
    global labelSet
    if len(detectedChar.shape)==3:
        this_img = cv2.cvtColor(detectedChar,cv2.COLOR_BGR2GRAY)
    else:
        this_img = detectedChar
    ...
    return labelSet[model.predict([img_50])[0]]
...
# if __name__ == "__main__":
    ...
    # for img_name in os.listdir('Test'):
        ...
        for box in bienso:
            pts= orderPoints(box)
            new_img = fourPointTransform(img,pts)
            ...
            try:
                if len(foundnum)>=8:
                    ...
                    for num in foundnum:
                        label = predict(num)
                        foundPlate+=label
                    cv2.putText(img,foundPlate,(int(bot_right[0]),int(bot_right[1])),cv2.FONT_HERSHEY_COMPLEX,.7,(0,0,255),2)
                    # print(foundPlate)
