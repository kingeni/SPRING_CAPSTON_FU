...
parser.add_argument("-t", "--test_img", help="Test image directory",
                    type=str, default=".\\DB\\test\\")
                    # type=str, default="D:\\Google Drive\\job\\Do an\\Raspberry\\bienSo\\CRNN-Keras-master\\DB\\test\\")
...
for test_img in test_imgs:
    ...
    if pred_texts == test_img[0:-4]:
        acc += 1
    total += 1
    print("text:",pred_texts)
    print('Predicted: %s  /  Orginal: %s' % (label_to_hangul(pred_texts), label_to_hangul(test_img[0:-4])))
    # print('Predicted: %s  /  Orginal: %s' % (label_to_hangul(pred_texts), label_to_hangul(test_img[0:-4])))
